...
export function localizedCancelReason(t: TFunction, raw: string): string {
  ...
  const map: Record<string, string> = {
    ...
    owner_cancelled: 'booking.cancelReason.ownerCancelled',
    maintenance: 'booking.cancelReason.maintenance',
    scheduling_conflict: 'booking.cancelReason.schedulingConflict',
    double_booking: 'booking.cancelReason.doubleBooking',
