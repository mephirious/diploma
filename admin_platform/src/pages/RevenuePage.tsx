        ...
        <>
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <StatCard
              ...
              value={formatPrice(data.totals.owner_refund_minor ?? 0, currency)}
              accent="danger"
            />
            <StatCard
              icon={<Wallet size={20} />}
              label={t('revenue.positiveIncome')}
            ...
            <Card className="md:col-span-1">
              <CardBody>
                ...
                <div className="mt-1 text-xs text-muted-light dark:text-muted-dark">
                  {formatPrice(data.today.net_gross_minor ?? data.today.owner_net_minor, currency)} ·{' '}
                  {data.today.succeeded_count} {t('revenue.successCount').toLowerCase()}
                </div>
              </CardBody>
            </Card>
            <Card>
              <CardBody>
                <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-light dark:text-muted-dark">
                  ...
                  {t('revenue.refundsToday')}
                </div>
                <div className="mt-2 text-2xl font-extrabold text-danger">
                  {formatPrice(data.today.owner_refund_minor ?? 0, currency)}
                </div>
                <div className="mt-1 text-xs text-muted-light dark:text-muted-dark">
                  {data.today.refund_count ?? 0} {t('revenue.refundCount').toLowerCase()}
                </div>
              </CardBody>
            </Card>
            <Card>
                      ...
                      <AreaChart data={chartData} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
                        ...
                        <Tooltip
                          ...
                          formatter={(val: number, name: string) => [
                            ...
                            name === 'owner_refund_minor' ? t('revenue.refunds') : t('revenue.positiveIncome'),
                          ]}
                          labelFormatter={(_, p) => {
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <div className="text-[11px] text-muted-light dark:text-muted-dark">
                          ...
                          {(v.refund_count ?? 0) > 0
                            ? ` · ${t('revenue.refunds')}: ${formatPrice(v.owner_refund_minor ?? 0, currency)}`
                            : ''}
                        </div>
                      </div>
                ...
                <table className="w-full min-w-[640px] text-left text-sm">
                  <thead className="border-b border-black/5 bg-black/[0.02] text-[11px] font-bold uppercase tracking-wider text-muted-light dark:border-white/10 dark:bg-white/[0.04] dark:text-muted-dark">
                    <tr>
                      ...
                      <th className="px-5 py-3">{t('revenue.colStatus')}</th>
                      <th className="px-5 py-3">{t('revenue.colMethod')}</th>
                      <th className="px-5 py-3 text-right">{t('revenue.colGross')}</th>
                      <th className="px-5 py-3 text-right">{t('revenue.colRefund')}</th>
                    ...
                    {(data.recent ?? []).map((r) => (
                      <tr key={r.id} className="hover:bg-black/[0.02] dark:hover:bg-white/[0.04]">
                        <td className="whitespace-nowrap px-5 py-3 text-muted-light dark:text-muted-dark">
                          {formatDateTime(r.event_at || r.succeeded_at, i18n.language)}
                        </td>
                        ...
                        <td className="px-5 py-3">
                          <Badge tone={r.status === 'refunded' ? 'danger' : 'success'}>
                            {r.status === 'refunded' ? t('revenue.refunded') : t('revenue.paid')}
                          </Badge>
                        </td>
                        ...
                        <td className="px-5 py-3 text-right font-semibold text-danger">
                          {(r.owner_refund_minor ?? 0) > 0
                            ? formatPrice(r.owner_refund_minor, r.currency || currency)
                            : '—'}
                        </td>
                        <td
                          className={`px-5 py-3 text-right font-extrabold ${
                            r.status === 'refunded' ? 'text-danger' : 'text-success'
                          }`}
                        >
