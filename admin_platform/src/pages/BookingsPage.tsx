...
const PAGE_SIZE = 30;
const OWNER_CANCEL_REASONS: OwnerCancelReason[] = [
  'owner_cancelled',
  'maintenance',
  'scheduling_conflict',
  'double_booking',
...
export function BookingsPage() {
  ...
  const [cancelReason, setCancelReason] = useState<OwnerCancelReason>('owner_cancelled');
  ...
  function canOwnerCancel(b: ApiBooking) {
  ...
  return (
    <div>
      ...
      <Modal
        ...
        footer={
          detail && canOwnerCancel(detail) ? (
            <Button
              type="button"
              variant="danger"
              size="sm"
              onClick={() => {
                setCancelReason('owner_cancelled');
          ...
          <>
            ...
            <Button
              ...
              onClick={() => {
                ...
                void cancelMut
                  .mutateAsync({ id, reason: cancelReason })
                  .then(() => {
      ...
      >
        <label className="block text-sm font-semibold text-text-light dark:text-text-dark">
          ...
          <select
            value={cancelReason}
            onChange={(e) => setCancelReason(e.currentTarget.value as OwnerCancelReason)}
            className="mt-2 h-11 w-full rounded-lg border border-black/10 bg-surface-light px-3 text-sm font-semibold text-text-light dark:border-white/10 dark:bg-surface-dark dark:text-text-dark"
          >
            {OWNER_CANCEL_REASONS.map((reason) => (
              <option key={reason} value={reason}>
                {localizedCancelReason(t, reason)}
              </option>
            ))}
          </select>
        </label>
