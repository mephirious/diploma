...
import { PromoEditor } from '@/components/resource/PromoEditor';
import { BlackoutEditor } from '@/components/resource/BlackoutEditor';
import { SessionRulesEditor } from '@/components/resource/SessionRulesEditor';
...
import {
  useArchiveSessionTemplate,
  useCreateOwnerSession,
  useCreateSessionTemplate,
  useRegenerateTemplateInvite,
  useSessionTemplates,
  useUpdateSessionTemplate,
} from '@/hooks/useSessions';

type Tab = 'overview' | 'schedules' | 'pricing' | 'promos' | 'blackouts' | 'sessions';
...
export function ResourceDetailPage() {
  ...
  const { data: templatesData } = useSessionTemplates(facilityId, {
    resource_id: resourceId,
    page_size: 100,
  });
  const createTemplate = useCreateSessionTemplate(facilityId);
  const updateTemplate = useUpdateSessionTemplate(facilityId);
  const archiveTemplate = useArchiveSessionTemplate(facilityId);
  const regenerateInvite = useRegenerateTemplateInvite(facilityId);
  const createSession = useCreateOwnerSession(facilityId);

  const sessionTemplates = templatesData?.items ?? [];
  ...
  return (
    <div>
      ...
      {tab === 'overview' ? (
        ...
        <PromoEditor venueId={facilityId} resourceId={resourceId} />
      ) : tab === 'blackouts' ? (
        <div className="space-y-6">
          <BlackoutEditor
            resourceId={resourceId}
        ...
        <SessionRulesEditor
          ...
          onArchiveTemplate={async (id) => {
            await archiveTemplate.mutateAsync(id);
          }}
          onRegenerateInvite={async (id) => {
